...
import { sanitize } from "../_lib/helpers";
...
type EmployeeState = {
  ...
  getAllEmployees: () => Promise<Array<UserEmployeeProfile>>;
...
export const useEmployeesStore = create<EmployeeState>((set) => ({
      ...
      addEmployee: (employee: UserEmployeeProfile) => {
        set((state) => ({
          ...state,
          employees: [ ...state.employees.filter(_empl  => _empl?.userId !== employee.userId) , employee],
        }));
      },
      editEmployee: async (newEmployee: UserEmployeeProfile) => {
        ...
        set((state) => ({
          ...
          employees: [
            ...
            { ...newEmployee, ...sanitize(updatedUser) },
          ],
          isLoadingData: false,
        }));
        return updatedUser;
